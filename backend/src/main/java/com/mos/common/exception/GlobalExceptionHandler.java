...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleGameEventNotFound(GameEventNotFoundException ex) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ErrorResponse.of(HttpStatus.NOT_FOUND.value(), "Not Found", ex.getMessage()));
    }

    @ExceptionHandler(NumberNotFoundException.class)
    public ResponseEntity<ErrorResponse> handleNumberNotFound(NumberNotFoundException ex) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(ErrorResponse.of(HttpStatus.NOT_FOUND.value(), "Not Found", ex.getMessage()));
    }

    @ExceptionHandler(NumberSessionMismatchException.class)
    public ResponseEntity<ErrorResponse> handleNumberSessionMismatch(NumberSessionMismatchException ex) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(ErrorResponse.of(HttpStatus.BAD_REQUEST.value(), "Bad Request", ex.getMessage()));
    }

    @ExceptionHandler(DuplicateNumberValueException.class)
    public ResponseEntity<ErrorResponse> handleDuplicateNumberValue(DuplicateNumberValueException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(ErrorResponse.of(HttpStatus.CONFLICT.value(), "Conflict", ex.getMessage()));
    }
    ...
    public ResponseEntity<ErrorResponse> handleQuestSessionMismatch(QuestSessionMismatchException ex) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(ErrorResponse.of(HttpStatus.BAD_REQUEST.value(), "Bad Request", ex.getMessage()));
    }

    @ExceptionHandler(QuestAlreadyCompletedException.class)
    public ResponseEntity<ErrorResponse> handleQuestAlreadyCompleted(QuestAlreadyCompletedException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(ErrorResponse.of(HttpStatus.CONFLICT.value(), "Conflict", ex.getMessage()));
    }

    @ExceptionHandler(QuestNotActiveException.class)
    public ResponseEntity<ErrorResponse> handleQuestNotActive(QuestNotActiveException ex) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(ErrorResponse.of(HttpStatus.BAD_REQUEST.value(), "Bad Request", ex.getMessage()));
    }
    ...
    public ResponseEntity<ErrorResponse> handleTradeNotParticipant(TradeNotParticipantException ex) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body(ErrorResponse.of(HttpStatus.FORBIDDEN.value(), "Forbidden", ex.getMessage()));
    }

    @ExceptionHandler(TradeInvalidStateException.class)
    public ResponseEntity<ErrorResponse> handleTradeInvalidState(TradeInvalidStateException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(ErrorResponse.of(HttpStatus.CONFLICT.value(), "Conflict", ex.getMessage()));
    }

    @ExceptionHandler(TradeItemNotOwnedException.class)
    public ResponseEntity<ErrorResponse> handleTradeItemNotOwned(TradeItemNotOwnedException ex) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body(ErrorResponse.of(HttpStatus.FORBIDDEN.value(), "Forbidden", ex.getMessage()));
    }

    @ExceptionHandler(TradeInsufficientCoinsException.class)
    public ResponseEntity<ErrorResponse> handleTradeInsufficientCoins(TradeInsufficientCoinsException ex) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(ErrorResponse.of(HttpStatus.BAD_REQUEST.value(), "Bad Request", ex.getMessage()));
    }
